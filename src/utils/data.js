...
前段时间跟着大佬写了一个CTF博客，前端是Vue写的。写完就想把它部署到网上，这样随时随地都能访问自己的学习笔记了。

一开始雄心勃勃想搞个Nginx服务器部署，研究了两天发现太麻烦了，要租服务器、配域名、搞SSL、弄防火墙...最后选择了**GitHub Pages**，免费还省事。这篇就记录一下我的部署历程和踩过的坑。
...
Vue Router用的History模式，刷新页面会404，需要配置\`try_files\`：
...
server {
    ...
    location / {
        ...
        index index.html;
        try_files $uri $uri/ /index.html;  # 这行很关键！
    }
}
\`\`\`
...
等待几分钟，会显示一个链接：\`https://你的用户名.github.io/ctf-blog/\`
...
| 你的情况 | 推荐方案 |
|---------|---------|
| 想省钱、怕麻烦 | **GitHub Pages** |
| 想练手Linux/服务器 | Nginx自建 |
| 国内访问速度要求高 | 可以考虑Vercel，或者用国内服务器 |
| 自定义域名+专业一点 | Nginx + 云服务器 |
...
现在我的博客已经跑起来了，虽然访问速度一般，但免费又省心，还要啥自行车！🚲
